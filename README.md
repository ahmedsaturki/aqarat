...
The dashboard data endpoint accepts bounded `limit` and `offset` parameters (defaults 50, maximum 100, maximum offset 1,000,000) and returns per-view `pagination` metadata with `returned`, `has_more`, and `next_offset`. Dashboard errors return a generic retryable code plus the request correlation identifier; the operator UI renders a localized retry message instead of exposing upstream details. The private operator surface also uses semantic navigation, a skip link, accessible labels and live status regions, visible keyboard focus, reduced-motion support, bounded login feedback, a same-origin browser check for login when origin metadata is present, and an idempotent logout that expires the HttpOnly session cookie. Dashboard Supabase reads use the shared outbound timeout helper rather than unbounded fetches. Dashboard sections support bounded Unicode-safe search over explicitly allowlisted fields; search input is capped at 80 characters and encoded before reaching Supabase, while overview and insights aggregations remain unfiltered by design. The repository quality gates are `npm run check`, which runs the full Node test suite and syntax validation for all application, API, script, and test modules, and `npm run verify:backup`, a read-only recovery-readiness contract that validates the runbook, migration ordering and timestamp uniqueness, secret-like patterns in migrations, and references to critical environment names without connecting to production or printing secret values. Production verification runs this gate before checking the public release contract and authenticated deep health. Gemini deep health uses a quota-safe model metadata probe by default: it validates the API key, model availability, and `generateContent` support without consuming generation quota. Set `GEMINI_HEALTH_PROBE=generation` only for an explicit diagnostic that needs to exercise generation; provider failures remain explicit failures and are never converted into success. Scheduled worker workflows use least-privilege GitHub permissions and single-flight concurrency controls to avoid overlapping queue processors. The Sheets worker invokes the existing `requeue_expired_jobs` RPC before claiming work, so expired leases can return to the queue; if this maintenance call is temporarily unavailable, the worker logs a bounded maintenance event and continues attempting claims rather than blocking all processing.
